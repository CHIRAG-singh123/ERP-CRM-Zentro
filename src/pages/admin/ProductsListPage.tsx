...
import { Plus, Search, Edit, Trash2, X, Filter, XCircle, Eye, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function AdminProductsListPage() {
  ...
  const [productFilter, setProductFilter] = useState<'my' | 'all' | string>('all');
  ...
  const { data, isLoading } = useProducts({
    ...
    limit,
    search,
    createdBy: productFilter === 'my' ? user?._id : productFilter !== 'all' ? productFilter : undefined,
  });

  const { data: usersData } = useAllUsers({ page: 1, limit: 100, isActive: true });
  const users = usersData?.users || [];

  // Get selected user info for display
  const selectedUser = productFilter && productFilter !== 'my' && productFilter !== 'all' 
    ? users.find(u => u._id === productFilter)
    : null;

  // Filter users: exclude customers, only show admin and employee/users
  const filteredUsers = users
    .filter(u => u.role !== 'customer') // Exclude customers
    .filter(u => {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex gap-4">
        ...
        <div className="relative flex items-center gap-2" ref={userSelectorRef}>
          <Filter className="h-4 w-4 text-white/50" />
          
          {/* Filter Type Selector */}
          <div className="relative">
            <select
              value={productFilter === 'my' || productFilter === 'all' ? productFilter : 'user'}
              onChange={(e) => {
              ...
              className="rounded-lg border border-white/10 bg-[#1A1A1C]/70 px-4 py-2 text-sm text-white focus:border-[#A8DADC] focus:outline-none focus:ring-2 focus:ring-[#A8DADC]/20 transition-all duration-200 cursor-pointer hover:border-white/20 min-w-[140px]"
            >
              ...
              <option value="user" className="bg-[#1A1A1C] text-white">
                Filter by User
              </option>
            </select>
          </div>
          ...
          {selectedUser && (
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg border border-[#A8DADC]/30 bg-[#A8DADC]/10">
              <UserAvatar
                avatarUrl={selectedUser.profile?.avatar}
                name={selectedUser.name}
                email={selectedUser.email}
                size={24}
              />
              <span className="text-sm text-[#A8DADC] font-medium">
                {selectedUser.name}
              </span>
              <button
                onClick={() => {
                ...
                className="text-[#A8DADC]/70 hover:text-[#A8DADC] transition-colors"
                title="Clear filter"
              >
          ...
          {showUserSelector && (
            <div className="absolute right-0 top-full mt-2 w-80 rounded-lg border border-white/10 bg-[#1A1A1C] shadow-2xl z-50 max-h-96 flex flex-col">
              {/* Search Input */}
              <div className="p-3 border-b border-white/10">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-white/10 bg-[#1A1A1C]/70 px-10 py-2 text-sm text-white placeholder:text-white/40 focus:border-[#A8DADC] focus:outline-none focus:ring-2 focus:ring-[#A8DADC]/20"
                    autoFocus
                  />
                </div>
              </div>
              ...
              {/* User List */}
              <div className="overflow-y-auto max-h-80 datagrid-scrollable">
                  ...
                  <div className="p-2">
                    {filteredUsers.map((u) => (
                      <button
                        key={u._id}
                        onClick={() => {
                          setProductFilter(u._id);
                          setShowUserSelector(false);
                          setUserSearchQuery('');
                        }}
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left text-sm text-white/70 hover:bg-white/5 hover:text-white transition-all duration-200 group"
                      >
                        <UserAvatar
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-medium text-white group-hover:text-[#A8DADC] truncate">
                            {u.name}
                          </div>
